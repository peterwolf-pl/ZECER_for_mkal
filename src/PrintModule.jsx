...
const getLineHeight = (line) => {
  ...
  return Math.max(
    ...line.map((l) =>
      l.height ? l.height * LETTER_SCALE : BASE_LETTER_HEIGHT
    )
  );
};
...
export default function PrintModule({ lines = [], onBack }) {
  ...
  // Pobranie easter eggow z bazy (API)
  useEffect(() => {
    async function loadEasterEggs() {
      try {
        ...
        if (!res.ok) {
          console.error("Easter eggs GET status:", res.status);
          return;
        }
        ...
        setEggs(
          list
            .map((e, index) => ({
              id: e.id || index,
              trigger: e.trigger || e.trigger_word || "",
              url: e.url || e.src || "",
              type: e.type || e.kind || "",
              description: e.description || "",
              word: e.trigger || e.trigger_word || "",
              src: e.url || e.src || "",
            }))
            .filter((e) => e.trigger)
        );
      } catch (err) {
        console.error("Easter eggs GET error:", err);
      }
    }
  ...
  // Mapowanie wyzwalaczy do easter eggow:
  // klucz = reverse(normalize(trigger)), lookup = normalize(tekstu linii)
  const easterEggMap = useMemo(() => {
    const map = new Map();
    eggs.forEach((egg) => {
      const base =
        egg.trigger ||
      ...
      const key = reverseString(normalized);
      if (key && !map.has(key)) {
        map.set(key, egg);
      }
    });
    ...
    if (!html2CanvasLoaderRef.current) {
      html2CanvasLoaderRef.current = new Promise((resolve, reject) => {
        ...
        const handleError = (event) => {
          ...
          reject(
            event?.error ||
              new Error("Nie udalo sie wczytac html2canvas")
          );
        };
  ...
  // Dynamiczne skalowanie dwoch kartek
  useEffect(() => {
    function handleResize() {
      ...
      setPageW(
        Math.min(A4_WIDTH, (maxW - 48) / 2, byHeight / 2, A4_WIDTH)
      );
    }
    handleResize();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () => setRightPageShown(true),
      1000
    );
    return () => clearTimeout(t);
  }, [animReady]);

  // Trigger easter egga po pokazaniu prawej strony
  useEffect(() => {
    if (!rightPageShown || easterEggMap.size === 0) {
      return;
    }
  ...
  }, [rightPageShown, safeLines, easterEggMap]);
  ...
  const handlePrintRightPage = useCallback(async () => {
    ...
    if (!rightPageEl) {
      console.warn(
        "[PrintModule] Nie znaleziono prawej strony do wydruku."
      );
      return;
    }
    ...
    try {
      ...
      if (document.fonts && document.fonts.ready) {
        try {
          ...
          console.warn(
            "[PrintModule] Nie udalo sie odczytac stanu czcionek",
            err
          );
        }
      }

      const prevTransform = rightPageEl.style.transform;
      const prevAnimation = rightPageEl.style.animation;
      const prevTransition = rightPageEl.style.transition;
      ...
      try {
        ...
        rightPageEl.style.transform = prevTransform;
        rightPageEl.style.animation = prevAnimation;
        rightPageEl.style.transition = prevTransition;
      }

      if (!canvas) {
        throw new Error(
          "Nie udalo sie wykonac zrzutu ekranu"
        );
      }
...
<html lang="pl">
  ...
  <body>
    <div class="sheet">
      <img id="print-image" src="${objectUrl}" alt="Podglad prawej strony" />
    </div>
    <script>
      (function() {
        const finish = () => {
          ...
          try {
            parent.postMessage(
              { type: 'print-module-right-page-finished' },
              '*'
            );
          } catch (_) {}
        };
        ...
        const startPrint = () => { setTimeout(triggerPrint, 50); };
      ...
      const handleMessage = (event) => {
        ...
        if (
          event.data.type ===
          "print-module-right-page-finished"
        ) {
          cleanup();
        }
      };
      ...
      console.error(
        "[PrintModule] Nie udalo sie przygotowac wydruku:",
        error
      );
      if (cleanupPrintIframeRef.current) {
        cleanupPrintIframeRef.current();
      ...
      >
        ...
        >
          <button
            ...
            title="Drukuj prawa strone"
            aria-label="Drukuj prawa strone"
          >
            🖨️
          </button>
